...
class AllSprites(pygame.sprite.Group):
    ...
    def draw(self, target_pos, dt):
        ...
        for sprite in sorted(self, key=lambda sprite: sprite.z):
            # Scale sprite position and size
            scaled_rect = pygame.Rect(sprite.rect.left * scale, sprite.rect.top * scale,
                                    sprite.rect.width * scale, sprite.rect.height * scale)
            # Apply offset
            offset_rect = scaled_rect.move(offset_x, offset_y)
            self.display_surface.blit(pygame.transform.scale(sprite.image, (scaled_rect.width, scaled_rect.height)),
                                    offset_rect.topleft)
